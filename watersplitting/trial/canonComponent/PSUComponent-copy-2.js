...
class PSUComponent extends THREE.Object3D {
  ...
  async init() {
    ...
    loader.load(
      ...
      async (loadedFont) => {
        ...
        await this.setupKnobs(); // Ensure knobs are set up before accessing
      },
      undefined,
      (error) => {
  ...
  async setupKnobs() {
    ...
    try {
      ...
      // Update positions for direct access
      this.update();
    } catch (error) {
  ...
  update() {
    if (this.redRing && this.blueRing) {
      this.redRingPosition = this.redRing.position.clone();
      this.blueRingPosition = this.blueRing.position.clone();
    }
  }
