...
class PSUComponent extends THREE.Object3D {
  constructor(
    ...
    camera, // Pass camera into the component for raycasting
    voltage = 12.34,
    current = 5.67,
    ...
    this.world = world;
    this.camera = camera; // Save the camera reference for raycasting
  ...
  onMouseDown(event) {
    ...
    raycaster.setFromCamera(mouse, this.camera); // Use this.camera for raycasting
    ...
    if (intersects.length > 0) {
      ...
      if (this.knobMap.has(clickedObject)) {
        ...
        if (knobType === "voltage") {
          this.voltage = (this.voltage + 1) % 13; // Increment and reset voltage
          this.updateDisplay(this.voltage, this.current); // Update display
        } else if (knobType === "current") {
          this.current = (this.current + 1) % 6; // Increment and reset current
          this.updateDisplay(this.voltage, this.current); // Update display
        }
      }
    }
