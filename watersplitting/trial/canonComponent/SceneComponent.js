...
export default class SceneComponent {
  ...
  createJar() {
    ...
    // Simplified material (no shaders)
    ...
    const thickGlassMaterial = new THREE.MeshBasicMaterial({
      color: 0x00ffff,
      transparent: true,
      opacity: 0.6,
    });
  ...
  createElectrodes() {
    const electrodeMaterial = new THREE.MeshBasicMaterial({ color: 0x002868 });

    const negativePlateGeometry = new THREE.BoxGeometry(0.1, 6, 2);
    const negativePlateMaterial = new THREE.MeshBasicMaterial({
      color: 0x002868,
    });
    ...
    const positivePlateMaterial = new THREE.MeshBasicMaterial({
      color: 0xbf0a30,
    });
