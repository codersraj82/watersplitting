...
const ELECTRODE_BUBBLE_DENSITY = 0.02; // Reduced density of bubbles around electrodes
...
function generateBubbles() {
  const midX = (negativePlate.position.x + positivePlate.position.x) / 2;
  const midY = (negativePlate.position.y + positivePlate.position.y) / 2;
  const midZ = (negativePlate.position.z + positivePlate.position.z) / 2;

  // Generate blue bubbles between electrodes
  for (let i = 0; i < ELECTRODE_BUBBLE_DENSITY * MAX_BUBBLES; i++) {
    const bubble = new THREE.Mesh(bubbleGeometry, blueBubbleMaterial.clone());
    bubble.position.set(
      midX + Math.random() * 1 - 0.5, // Random X within the range between electrodes
      midY + Math.random() * 1 - 0.5, // Random Y within the range between electrodes
      midZ + Math.random() * 1 - 0.5 // Random Z within the range between electrodes
    );
    ...
    if (bubble.position.y >= -2.5 && bubble.position.y <= 1.25) {
      ...
      scene.add(bubble);
      bubbles.push({ bubble, target: negativePlate.position });
    }
  }

  // Generate red bubbles between electrodes
  for (let i = 0; i < ELECTRODE_BUBBLE_DENSITY * MAX_BUBBLES; i++) {
    const bubble = new THREE.Mesh(bubbleGeometry, redBubbleMaterial.clone());
    bubble.position.set(
      midX + Math.random() * 1 - 0.5, // Random X within the range between electrodes
      midY + Math.random() * 1 - 0.5, // Random Y within the range between electrodes
      midZ + Math.random() * 1 - 0.5 // Random Z within the range between electrodes
    ...
    if (bubble.position.y >= -2.5 && bubble.position.y <= 1.25) {
      ...
      bubble.receiveShadow = true;
      scene.add(bubble);
      bubbles.push({ bubble, target: positivePlate.position });
    }
  }

  // Generate white bubbles throughout the liquid
  for (let i = 0; i < CENTER_BUBBLE_DENSITY * MAX_BUBBLES; i++) {
    const bubble = new THREE.Mesh(bubbleGeometry, whiteBubbleMaterial.clone());
    bubble.position.set(
      Math.random() * 5.8 - 2.9, // Random X within the liquid width
      Math.random() * 3 - 1.5, // Random Y within the liquid height
      Math.random() * 5.8 - 2.9 // Random Z within the liquid depth
    );
    ...
    if (bubble.position.y >= -2.5 && bubble.position.y <= 1.25) {
      ...
      bubble.receiveShadow = true;
      scene.add(bubble);
      bubbles.push({ bubble, target: null }); // White bubbles do not have a target
    }
  }
}
...
function updateBubbles() {
  bubbles.forEach((bubbleObj) => {
    const { bubble, target } = bubbleObj;

    if (target) {
      // Move towards the target electrode
      const direction = new THREE.Vector3()
        .subVectors(target, bubble.position)
        .normalize();
      bubble.position.add(direction.multiplyScalar(BUBBLE_SPEED));
    } else {
      // White bubbles move upwards
      bubble.position.y += BUBBLE_SPEED;

      // Regenerate white bubbles if they move out of bounds
      if (bubble.position.y > 1.25) {
        bubble.position.set(
          Math.random() * 5.8 - 2.9, // Reset X within the liquid width
          -2.5, // Start position at the bottom of the liquid
          Math.random() * 5.8 - 2.9 // Reset Z within the liquid depth
        );
      }
    }
    ...
    if (bubble.position.y > 1.25) {
      scene.remove(bubble);
      bubbles.splice(bubbles.indexOf(bubbleObj), 1);
      return;
    }
