...
class PSUComponent {
  ...
  getRingPositions() {
    const redRing = this.psuGroup.children.find(
      (child) => child.material && child.material.color.getHex() === 0xff0000
    );
    const blueRing = this.psuGroup.children.find(
      (child) => child.material && child.material.color.getHex() === 0x0000ff
    );
    return {
      redRingPosition: redRing ? redRing.position : null,
      blueRingPosition: blueRing ? blueRing.position : null,
    };
  }
