...
function generateBubbles() {
  ...
  if (currentTime - lastBubbleTime > BUBBLE_INTERVAL * 10) {
    ...
    if (bubbles.length < MAX_BUBBLES) {
      // Generate white bubbles uniformly throughout the liquid
      const xWhite = Math.random() * 6 - 3;
      const yWhite = Math.random() * 6 - 3;
      const zWhite = Math.random() * 3 - 1.5;

      const whiteBubble = new THREE.Mesh(bubbleGeometry, whiteBubbleMaterial);
      whiteBubble.position.set(xWhite, yWhite, zWhite);
      scene.add(whiteBubble);
      bubbles.push({
        mesh: whiteBubble,
        target: new THREE.Vector3(0, 2, 0), // Move up
        color: "white",
      });

      // Generate blue and red bubbles between electrodes
      const electrodes = [
        {
          color: "blue",
          electrode: negativePlate,
          density: ELECTRODE_BUBBLE_DENSITY,
        },
        {
          color: "red",
